...
class SessionFrame(Frame):
    ...
    def take(self):# take photo with raspberry gpio
        try:
            os.system(f"python {self.pth}/take.py")
        except:
            messagebox.showinfo("warning", "camera not available")
        """
        self.frame.destroy()
    ...
    def showgrid(self):
        ...
        self.button1 = Button(self.frame1, text="calculate length and identify", command=self.calculate)
...
class UnsentFrame(Frame):
    ...
    def display(self):
        ...
        k = 0
        for i in index:
            Label(self.frame1.viewPort,width=25, height=2,text=i, relief=RIDGE, bg="gray",).grid(row=0, column=k)
            k += 1
        r = 1
        for row in self.data:
            c = 0
            for col in row:
                if c == 0:
                    Button(self.frame1.viewPort, width=25,height=2, text=self.getname(col),relief=RIDGE,command=Callback(self.show, col),).grid(row=r, column=c)
                else:
                    Label(self.frame1.viewPort,width=25,height=2,text=self.name(col),relief=RIDGE,).grid(row=r, column=c)
                c += 1
            Button(self.frame1.viewPort,width=25,height=2,text="delete",relief=RIDGE,command=Callback(self.delete, r), ).grid(row=r, column=c)
            r += 1
